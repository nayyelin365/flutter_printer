...
class TsplPrinterScreen extends StatefulWidget {
  const TsplPrinterScreen({super.key});

  @override
  State<TsplPrinterScreen> createState() => _TsplPrinterScreenState();
}

class _TsplPrinterScreenState extends State<TsplPrinterScreen> {
  ...
  Future<void> _printShippingLabel() async {
    ...
    try {
      final labelData = TsplCommands.sampleShippingLabel();
      final success = await _printerService.printTspl(labelData);

      if (success) {
        _showSuccess('Shipping label printed!');
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('TSPL Label Printer'),
            ...
            itemBuilder: (context, index) {
              ...
              return Card(
                ...
                child: ListTile(
                  leading: Icon(
                    Icons.label,
                    color: isConnected ? Colors.green : Colors.green.shade700,
                  ),
